...
def create_school_student_debts(group, students):
    ...
    for student in students:
        ...
        for date in datas:
            ...
            print(date)
            per_month, created = AttendancePerMonth.objects.get_or_create(
                month_date=month_date,
                student=student,
                group=group,
                system=group.system,
                defaults={'total_debt': group.class_number.price}
            )
            print(per_month, created)
...
def generate_months(start_year, start_month, end_year, end_month):
    ...
    if 1 <= start_month <= 6:
        ...
        current_year = start_year
        current_month = start_month
        while current_year < end_year or (current_year == end_year and current_month <= 6):
            dates.append((current_year, current_month))
            if current_month == 12:
                current_month = 1
                current_year += 1
